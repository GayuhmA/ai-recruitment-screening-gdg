...
async function resolvePdfParseFn(): Promise<
  (data: Uint8Array) => Promise<PdfParseResult>
> {
  ...
  const candidates = [mod?.default, mod, mod?.pdfParse];
  ...
  throw new Error(
    `pdf-parse export is not a function. Keys: ${keys.join(', ')}`,
  );
}
